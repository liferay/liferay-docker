...
function _build_docker_image {
	...
	sed --expression "s/@jdk_from_image_name@/${jdk_from_image_name}/g" --in-place "${TEMP_DIR}/Dockerfile"
	sed --expression "s/@jdk_version@/${jdk_version}/g" --in-place "${TEMP_DIR}/Dockerfile"
