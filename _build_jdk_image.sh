...
function _build_docker_image {
	...
	local jdk_image_name="${3}"
	local jdk_version_number="${4}"

	delete_local_images "${LIFERAY_DOCKER_REPOSITORY}/${jdk_image_name}"

	make_temp_directory templates/${jdk_image_name} templates/_jdk
	...
	local zulu_amd64_version="LIFERAY_DOCKER_ZULU_${jdk_version_number}_AMD64_VERSION"
	local zulu_arm64_version="LIFERAY_DOCKER_ZULU_${jdk_version_number}_ARM64_VERSION"
	...
	then
		...
		docker buildx build \
			...
			--build-arg LABEL_ZULU_${jdk_version_number}_AMD64_VERSION="${!zulu_amd64_version}" \
			--build-arg LABEL_ZULU_${jdk_version_number}_ARM64_VERSION="${!zulu_arm64_version}" \
