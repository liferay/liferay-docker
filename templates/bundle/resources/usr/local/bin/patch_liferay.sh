...
function main {
	if [ -n "$(ls --almost-all "${LIFERAY_PATCHING_DIR}"/patching-tool-*.zip 2> /dev/null)" ]
	then
		echo ""
	...
	elif [ -d "${LIFERAY_PATCHING_DIR}" ] && [ -n "$(find "${LIFERAY_PATCHING_DIR}" -maxdepth 1 -type f -name "*liferay-*.zip" 2> /dev/null)" ]
	then
